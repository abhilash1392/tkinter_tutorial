...
def button_equal():
    ...
    if math=='divide':
        if int(second_number)==0:
            e.inset(0,text='Cannot divide by zero')
        else:
            e.insert(0, f_num/float(second_number))
...
button_clear= Button(root,text='CLEAR',padx=78,pady=20,command=button_clear).grid(row=4,column=1,columnspan=2)
button_add= Button(root,text='+',padx=40,pady=20,command=button_add).grid(row=5,column=0)
button_sub= Button(root,text='-',padx=40,pady=20,command=button_sub).grid(row=5,column=1)
button_prod= Button(root,text='*',padx=40,pady=20,command=button_prod).grid(row=5,column=2)
button_div= Button(root,text='/',padx=40,pady=20,command=button_div).grid(row=6,column=0)
button_equal= Button(root,text='=',padx=90,pady=20,command=button_equal).grid(row=6,column=1,columnspan=2)
